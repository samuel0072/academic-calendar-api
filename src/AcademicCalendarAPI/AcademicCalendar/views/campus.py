...
def edit_campus(request, id):
    try:
        ...
        if serializer.is_valid():
            ...
            return Response(serializer.errors, status=status.HTTP_422_UNPROCESSABLE_ENTITY, content_type="application/json") 
        
    except Campus.DoesNotExist:
        return Response({"errors": [_('Could not find the campus.')]},  status=status.HTTP_404_NOT_FOUND, content_type="application/json")
    
    except AcademicCalendarException as err:
        return Response(err.args, status=status.HTTP_400_BAD_REQUEST, content_type="application/json")
...
def delete_campus(request, id):
    try:
        ...
        if events_count > 0:
            return Response({"event": [_('This campus has events associated. You should desaasiate those events before deleting this campus.')] }, status=status.HTTP_400_BAD_REQUEST, content_type="application/json")
        ...
        return Response({"errors": err.args }, status=status.HTTP_400_BAD_REQUEST, content_type="application/json")
